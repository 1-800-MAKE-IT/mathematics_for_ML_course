...
# This next line defines our error flag. Inherits from base exception class. For when things go wrong if the matrix is singular.
...
def fixRowZero(A) :
    if A[0,0] == 0 :
        A[0] = A[0] + A[1]
    #if still zero, we add the next row.
    if A[0,0] == 0 :
        A[0] = A[0] + A[2]
    if A[0,0] == 0 :
        A[0] = A[0] + A[3]
